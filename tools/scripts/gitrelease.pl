...
sub usage {
  return
    "gitrelease.pl WORKSPACE VERSION [STEPS] [options]\n" .
...
sub parse_version {
  ...
  if ($version =~ /^(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?$/) {
    ...
    $result{series} = "$result{major}.$result{minor}";
    $result{release_string} = "$result{series}.$result{micro}";
    $result{orig_string} = "$version";
  }
  return %result;
}
...
sub remedy_update_version_h_file {
  ...
  my $parsed_version = $post_release ?
    $settings->{parsed_next_version} : $settings->{parsed_version};
  my $version = $parsed_version->{orig_string};
  ...
  my $release_line = "#define OPENDDS_IS_RELEASE $parsed_version->{release}";
...
if ($next_version) {
  ...
  if (!%parsed_next_version) {
    die "Invalid argument to --next-version: $next_version";
  }
}
...
if (%parsed_version) {
  ...
  if (!$micro) {
    $release_branch =
      "${release_branch_prefix}${git_name_prefix}$parsed_version{series}";
  ...
  if (!$next_version) {
    $next_version = sprintf("%d.%d.%d",
      $parsed_version{major}, $parsed_version{minor} + 1, 0);
  }
...
my %global_settings = (
    ...
    parsed_next_version => \%parsed_version,
    base_name    => $base_name,
